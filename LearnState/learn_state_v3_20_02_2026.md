...
| Фаза 1: Математика | **~68%** | Сильное ядро (MLE, Байес, backprop, SVD/PCA, CLT/LLN). Начал матстатистику (прикладная статистика, неделя 6). Пробелы: CI, тесты гипотез, A/B, оптимизаторы |
...
### 1.2 Теория вероятностей и статистика (~70%)

#### Прогресс по курсу «Прикладная статистика»

| Неделя | Тема | Статус |
|--------|------|--------|
| 1 | Случайные величины, описательные статистики | ✅ Пройдено |
| 2 | Ковариация, корреляция, нормальное распределение | ✅ Пройдено |
| 3 | Парсинг данных | ⏭️ Пропущено (не для ML) |
| 4 | EDA, визуализация | ⏭️ Пропущено (не для ML) |
| 5 | CLT, LLN, сходимости, проклятие размерности | ✅ **Пройдено глубоко** |
| **6** | **Схема матстатистики, CI для средних и долей** | **🔄 Сейчас здесь** |
| 7 | Свойства оценок, Bias-Variance | ❌ |
| 8 | Точные CI (t, χ², F) | ❌ |
| 9 | Проверка гипотез, p-value | ❌ |
| 10 | Бутстрап, непараметрические тесты | ❌ |
| 11 | A/B-тесты | ❌ |
| 12 | MLE (статистический взгляд) | ❌ |
| 13 | MLE↔Loss Functions, Bias-Variance | ❌ |
| 14 | Временные ряды | ⏭️ Низкий приоритет |
| 15 | SARIMA, TBATS | ⏭️ Пропустить |
...
- **ЦПТ (Central Limit Theorem)** — **очень глубоко**: формулировка, визуализация, √n(X̄−μ)/σ →ᵈ N(0,1), почему нормальное распределение встречается повсюду, фундамент для CI и тестов гипотез
- **ЗБЧ (Law of Large Numbers)** — **очень глубоко**: X̄_n →ᵖ μ, визуализация, связь с симуляциями Монте-Карло

**✅ Знаю базово:**
- **Нормальное распределение** — стандартизация Z = (X−μ)/σ, z-таблица (Φ), правило 68-95-99.7, многомерное нормальное
- **Энтропия Шеннона** — мера неопределённости, формула H = -Σ p·ln(p)
- **Ковариация и корреляция** — Cov(X,Y), Corr ∈ [−1,1], корреляция ≠ зависимость
- **χ², t, F-распределения** — знакомство: «друзья» нормального, зачем нужны для статистических тестов
- **Проклятие размерности** — объём шара → 0 при d→∞, KNN перестаёт работать
- **Монте-Карло симуляции** — через ЗБЧ
- **Виды сходимостей** — по вероятности, по распределению, п.н. (обзорно)

**⚠️ Частично:**
- Условная вероятность — через Байеса, но формально совместные распределения и маргинализацию не разбирал

**❌ Не знаю (следующие темы в курсе прикладной статистики):**
- Confidence intervals (доверительные интервалы) — **неделя 6, начинаю сейчас**
- Свойства оценок: несмещённость, состоятельность — неделя 7
- Bias-Variance Tradeoff (математическая декомпозиция) — недели 7, 13
- Hypothesis testing (t-test, p-value, ошибки I/II рода) — неделя 9
- Bootstrap — неделя 10
- A/B-тестирование — неделя 11
- MLE (статистический взгляд: информация Фишера, likelihood ratio test) — неделя 12
- MLE↔Loss Functions (MSE=MLE при гауссовском шуме, и т.д.) — неделя 13
...
| Сформулируйте ЦПТ | ✅ глубоко |
| Сформулируйте ЗБЧ | ✅ глубоко |
| Почему нормальное распределение так часто встречается? | ✅ (ответ: ЦПТ) |
| Что такое проклятие размерности? | ✅ базово |

### Математика — НЕ могу ответить:

| Вопрос | Статус |
|--------|--------|
| Bias-variance tradeoff математически | ❌ (будет на неделях 7, 13) |
| Связь SVD и PCA | ✅ |
| Почему Adam лучше SGD | ❌ |
| Что такое p-value | ❌ (будет на неделе 9) |
| Как устроен A/B-тест | ❌ (будет на неделе 11) |
| Что такое доверительный интервал | ❌ (начинаю на неделе 6) |
...
*Обновить после прохождения недели 6 (CI) курса прикладной статистики.*
